import { redirect, useNavigate } from "react-router-dom";
import { setLogin } from "../../reducers/ProductsReducer";
import { useDispatch } from "react-redux";
function Login () {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const H1clickHandler = () => {
        navigate('/signup')
    }

    const loginButtonHandler = () => {
        dispatch(setLogin(true));
    }

        return (
            <div className="w-full h-fit bg-[#f1f3f6] p-3">
                    <div className="flex bg-red-200 w-[60%] h-[63vh] m-auto">

                        <div className="bg-[#2874f0] w-[40%] h-full p-5">
                              <div className="text-white mt-7 ml-3">
                                    <h1 className="text-3xl mb-5">Login</h1>
                                    <h5>Get access to your Orders,<br /> Wishlist and Recommendations</h5>
                              </div>
                              <img className="m-auto mt-30 rounded-md" src="data:image/png;base64," alt="" />
                        </div>

                        <div className="bg-[#ffffff] w-[60%] h-full">
                                <form action="/" className="ml-9 mt-7 w-[80%] m-auto text-center">
                                     <input className="w-full p-3 text-xl border-1  mb-3" type="email" placeholder="Email"/>
                                     <input className="w-full p-3 text-xl border-1 mb-3" type="password" placeholder="password"/>
                                     <h5 className="text-xs mb-3 text-gray-500">By continuing, you agree to Flipkart's Terms of Use and Privacy Policy.</h5>
                                     <button className="p-2 bg-[#fb641b] w-full rounded text-white " onClick={loginButtonHandler}>Login</button>
                                </form>

                                <h1 onClick={H1clickHandler} className="text-[#2874f0] text-center mt-10 cursor-pointer">New to Flipkart? Create an account</h1>
                        </div>
                    </div>
            </div>
        )
}


export default Login;