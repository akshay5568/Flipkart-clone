...
function Cart() {
  ...
  const increaceQty = async (title, _id) => {
    await axios.post(
      `${import.meta.env.VITE_BACKEND_URL}/inc-qty`,
      { inc, title },
      {
        headers: {
          Authorization: `Bearer ${token}`,
              ...
              return (
                <div className="m-auto p-5 px-7 mt-4 bg-[#ffffff]">
                  <div className="flex sm:gap-5">
                    ...
                    <div>
                      <h1 className="sm:text-normal text-sm">
                        {item.title
                      ...
                      <div className="flex gap-3 items-center mt-4 ">
                        <h1 className="sm:text-xl sm:font-normal font-semibold">
                          ₹{item.price}
                        </h1>
                      ...
                      <div className="w-[45%] flex justify-center items-center mt-3 gap-3">
                        <button
                          onClick={() => decreaseQty(item.title, item._id)}
                          className="bg-amber-300 px-2 rounded-full"
                        >
                          -
                        </button>
                        <h6 className="border-1 px-3 rounded">{item.qty}</h6>
                        <button
                          onClick={() => increaceQty(item.title, item._id)}
                          className="bg-amber-300 px-2 rounded-full"
                        >
                          +
                        </button>
